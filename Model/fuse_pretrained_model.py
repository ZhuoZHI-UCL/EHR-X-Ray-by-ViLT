#这个模型是用CXR_UNI预训练，然后用partiacal_ehr_cxr微调的
...
class fuse_pretrained_model(pl.LightningModule):
    def __init__(self, config):
        ...
        #从给定的路径加载预训练权重，在之前的模型里面都是加载的Vit的预训练权重, 现在我们要加载的是CXR_UNI的预训练权重
        if (
        ...
        self.cxr_classifier.apply(objectives.init_weights)  
    ...
    def forward(self, batch):
        ...
        if test_model == 'ehr':#比较好的参数 batchsize 256  lr 1e-4 auroc 0.82 NUM 11
            ...
            #-----------------------------------------------------返回结果-----------------------------------------------------#
            pre = self.cxr_classifier(cls_feats)
            return pre
    ...
    def configure_optimizers(self): #这个函数是用来配置优化器的
        ...
        head_names = ["cxr_classifier", "mmimdb_classifier", "food101_classifier", "hatememes_classifier", "nlvr2_classifier"]
        prompt_name = "prompt"
    ...
    def on_train_start(self):
        ...
        files_to_copy = {
            'fuse_config.py': '/scratch/uceezzz/Project/Mis_mul/Soluation_mis/Config/fuse_pretrained_config.py',
            'Extract_data_1.py': '/scratch/uceezzz/Project/Mis_mul/Soluation_mis/Dataloader/Extract_data_1.py',
            'fuse_model.py': '/scratch/uceezzz/Project/Mis_mul/Soluation_mis/Model/fuse_pretrained_model.py'
        }
